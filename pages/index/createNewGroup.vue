<template>
	<view>
		<u-navbar :is-back="true" title="创建群聊" :background="{ background: '#F6F7F8' }" title-color="#404133" :border-bottom="false" z-index="1001">
			<view class="slot-wrap" slot="right">
				<u-button style="margin-right: 30rpx;" size="mini" type="success" @click="createGroup()">创建</u-button>
			</view>
		</u-navbar>
		<view style="width:100%;margin-top: 10rpx;height: 80rpx;">
			<input style="margin-left: 40rpx;background-color: #F5F5F5;margin-right: 40rpx;height:70rpx;"type="text"  v-model="newGroup.name" placeholder="请输入群聊名称"/>
		</view>
		<!-- 我的群聊 -->
		<view style="width:100%;height: 60rpx;background-color: #F5F5F5	;"><p style="margin-left: 20rpx;">选择好友</p></view>
	    <scroll-view>
			<!-- 好友选择框 -->
			<view style="width:90%;margin-top: 10rpx;margin-left: 20rpx;margin-right: 20rpx;">
				<!-- <evan-checkbox-group > -->
				<view v-for="(item,index) in friendList" :key="index">
					<el-checkbox v-if="item.id!=loginUserInfo.id" v-model="item.check" :label="item.username" border size="medium"></el-checkbox>
...
<script>
	export default {
		...
		methods: {
			...
			createGroup(){
				//添加好友
				let addurl=this.$baseUrl+'/api/group/group?ids='+this.loginUserInfo.id+'&'
				console.log(addurl)
				console.log('Friend',this.friendList)
				this.friendList.forEach(item => {
					if(item.check==true){
						addurl=addurl+'ids='+item.id+'&'
					 }	
					})
					
				addurl=addurl.slice(0,addurl.length-1)
				console.log(addurl)
				this.newGroup.administratorId=this.loginUserInfo.id
				uni.request({
					url:addurl,
					method:'POST',
					data:{"administratorId":this.loginUserInfo.id},
					// params:{ids:this.ids},
					success:(res=>{
