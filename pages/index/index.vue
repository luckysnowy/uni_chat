<template>
	<scroll-view class="aui-news-box">
		<!-- <view class="aui-news-item" @click="linkToNewFriend()">
			<view class="aui-news-item-hd" >
				<image src="/static/friends.png"  class="img"/>
			</view>
			<view class="aui-news-item-bd">
				<h4>新的好友 </h4>
			</view>
		</view> -->
		<view class="aui-news-item" @click="linkToGroup()">
			<view class="aui-news-item-hd">
				<image src="/static/group.png"  class="img"/>
			</view>
			<view class="aui-news-item-bd">
				<h4>群聊</h4>
				<!-- <p>{{item.message}}</p> -->
		...
		<view style="width:100%;height: 60rpx;background-color: #F5F5F5	;"><p style="margin-left: 20rpx;">我的好友</p></view>
		<view v-for="(item, index) in contacts" :key="index" v-show="item.id!=loginUserInfo.id"class="aui-news-item" @click="linkTo(item)">
			<view class="aui-news-item-hd">
...
<script>
	export default {
		...
		methods: {
			...
			linkTo(item){
				this.$u.route({
					url: 'pages/index/friendInfo?user='+JSON.stringify(item)
				});
			},
			linkToNewFriend(){
				this.$u.route({
