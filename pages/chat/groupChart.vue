<template>
	<view>
		<u-navbar :is-back="true" :custom-back="linkToIndex" title="我的群聊" :background="{ background: '#F6F7F8' }" title-color="#404133" :border-bottom="false" z-index="1001">
			<view class="slot-wrap" slot="right">
				<u-button style="margin-right: 30rpx;" size="mini" type="success" @click="addGroup()">创建</u-button>
			</view>
		</u-navbar>
		<!-- 我的群聊 -->
		<view style="width:100%;height: 60rpx;background-color: #F5F5F5	;"><p style="margin-left: 20rpx;">我的群聊</p></view>
	    <scroll-view class="aui-news-box">
			<view v-for="(item, index) in groups" :key="index" class="aui-news-item" @click="linkTo(item)">
				<view class="aui-news-item-hd">
					<image src="/static/groupchat.png"  class="img"/>
				</view>
	...
	export default {
		...
		methods: {
			...
			linkTo(item){
				...
				this.$u.route({
					url: 'pages/index/groupInfo?id='+item.id
				});
			},
			addGroup(){
				this.$u.route({
					url: 'pages/index/createNewGroup'
				});
			},
			linkToIndex(){
				// this.$u.route({
