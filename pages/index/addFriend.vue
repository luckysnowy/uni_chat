<template>
	<view>
		<u-navbar :is-back="true" title="新的朋友" :background="{ background: '#F6F7F8' }" title-color="#404133" :border-bottom="false" z-index="1001">
			<view class="slot-wrap" slot="right">
				<u-button style="margin-right: 30rpx;" size="mini" type="success" @click="addFriend()">添加好友</u-button>
			</view>
		</u-navbar>
		<!-- 请求信息 -->
		<view style="width:100%;height: 60rpx;background-color: #F5F5F5	;"><p style="margin-left: 20rpx;">好友申请</p></view>
	    <scroll-view class="aui-news-box">
			<view v-for="(item, index) in list" :key="index" class="aui-news-item">
				<view class="aui-news-item-hd">
					<image src="/static/img2.jpg"  class="img"/>
				</view>
				...
				<view class="aui-news-item-fr">
					<u-button size="mini" style="margin-right: 10rpx;" type="success" @click="isAgreeFriendApply(1,item)">同意</u-button>
					<u-button size="mini" type="warning" @click="isAgreeFriendApply(2,item)">拒绝</u-button>
				</view>
			</view>
		</scroll-view>
	</view>
...
<script>
	export default {
		...
		methods: {
			addFriend(){
				this.$u.route({
					url: 'pages/index/addNewFriend'
				});
			},
			isAgreeFriendApply(type,item){
				if(type==1){
