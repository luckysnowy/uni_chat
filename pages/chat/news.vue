<template>
		<scroll-view class="aui-news-box">
			...
			<view v-for="(item, index) in chartsList" :key="index" class="aui-news-item" @click="linkTo(item)">
				<view class="aui-news-item-hd">
					<image src="/static/img2.jpg"  class="img"/>
				</view>
				<view class="aui-news-item-bd">
					<h4>{{item.username}} </h4>
					<p>{{item.showNews.content}}</p>
					<!-- <p>{{item.time}}</p> -->
				</view>
				<view class="aui-news-item-fr">
					<p>{{item.showNews.time}}</p>
				</view>
			</view>
		</scroll-view>
</template>
...
export default {
		...
		'socketMsgs': {
			...
			handler: function() {
				...
				if(sMsg!="没有传入id或者 此id已经在线"){
					let message=JSON.parse(sMsg)
					//获取sourceId的用户信息
					if(message.targetType==0){
						//私聊信息
							uni.request({
							  url:this.$baseUrl+'/api/user/user/'+message.sourceId,
							  method:'GET',
							  success:(res=>{
								  if(res.data.code==200){
									 this.newChartInfo=res.data.data
									 let isNotExist=true
									 let existindex=-1
									 this.chartsList.forEach((item,index)=>{
										 if(item.id==this.newChartInfo.id){
											 isNotExist=false
											 existindex=index
										 }
									 })
									 if(isNotExist==true){
										 this.newChartInfo.showNews=message
										 this.chartsList.push(this.newChartInfo)
									 }else if(isNotExist=false){
										 this.chartsList[index].showNews=message
									 }
								  }
							  })
							})
	...
	methods: {
		...
		linkTo(item){
			this.$u.route({
				url: 'pages/chat/chatsItem?user='+JSON.stringify(item)
			});
		}
	},
};
