	...
	mutations: {
		...
		webSocketInit(state) {
			...
			state.socketTask = uni.connectSocket({
				url: "ws://121.196.99.253:8081/groupWebSocket?userId="+state.uid+'&groupId='+state.sid,
				success(data) {
					console.log("websocket群聊连接成功");
				},
			});
		...
		// webSocketCloseAction(state) {
		// 	let that = this
		// 		if (!that.state.webSocketIsOpen) {
		// 			return false;
		// 		}
		// 	    that.state.socketTask.onClose((errMsg) => {
		// 		console.log(errMsg)
		// 		console.log("ws连接关闭")
		// 		state.webSocketIsOpen = false;
		// 		state.webSocket = null;
